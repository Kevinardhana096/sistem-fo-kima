...
async fn main() {
    ...
    let trusted_proxy_ips = Arc::new(parse_trusted_proxy_ips(
        &env::var("TRUSTED_PROXY_IPS").unwrap_or_else(|_| "127.0.0.1,::1".to_owned()),
    ));
    ...
    let state = Arc::new(AppState {
        ...
        max_upload_bytes: optional_env_usize("MAX_UPLOAD_BYTES", 25 * 1024 * 1024),
        max_string_length: optional_env_usize("MAX_STRING_LENGTH", 500),
        token_expiry_seconds: optional_env_u64("TOKEN_EXPIRY_SECONDS", 60 * 60 * 8),
        login_rate_limit: optional_env_usize("LOGIN_RATE_LIMIT", 5),
...
fn parse_trusted_proxy_ips(value: &str) -> HashSet<IpAddr> {
    value
        ...
        .filter_map(|item| {
            ...
            match item.parse() {
                Ok(ip) => Some(ip),
                Err(_) => panic!("TRUSTED_PROXY_IPS berisi alamat IP tidak valid: {item}"),
            }
        })
        .collect()
}
