...
pub async fn login(
    ...
    let ip = client_ip(addr.ip(), &headers, &state.trusted_proxy_ips).to_string();
...
mod tests {
    ...
    fn trusted_proxy_uses_forwarded_client_ip() {
        ...
        assert_eq!(
            client_ip(proxy, &headers, &trusted),
            "203.0.113.10".parse::<IpAddr>().unwrap()
        );
    }

    #[test]
    fn trusted_proxy_ignores_client_supplied_spoofed_prefix() {
        let proxy: IpAddr = "127.0.0.1".parse().unwrap();
        let trusted = HashSet::from([proxy]);
        let mut headers = HeaderMap::new();
        headers.insert(
            "x-forwarded-for",
            HeaderValue::from_static("192.0.2.99, 203.0.113.10"),
        );
    ...
    fn malformed_proxy_header_falls_back_to_peer() {
        ...
        assert_eq!(client_ip(proxy, &headers, &trusted), proxy);
    }
}
