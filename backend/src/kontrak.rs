...
pub async fn delete_contract(
    ...
    // Semua perubahan database harus berhasil atau dibatalkan bersama-sama.
    // Folder Drive dihapus setelah commit agar kegagalan database tidak
    // menghilangkan berkas yang masih direferensikan.
    ...
    let result = sqlx::query("DELETE FROM lokasi WHERE id = ?")
        .bind(id)
        .execute(&mut *tx)
        .await
        .map_err(|e| {
    ...
    if result.rows_affected() != 1 {
        return Err(ApiError::not_found("Kontrak tidak ditemukan"));
    }
    tx.commit().await.map_err(ApiError::database)?;

    if let Some(ref link) = link_folder
        && let Some(folder_id) = parse_drive_folder_id(link)
        && let Err(e) = delete_kontrak_tree(&state.drive, &folder_id).await
    {
        tracing::warn!(
            contract_id = id,
            folder_id = %folder_id,
            error = %e,
            "Kontrak sudah dihapus dari database, tetapi folder Drive gagal dibersihkan"
        );
    }
