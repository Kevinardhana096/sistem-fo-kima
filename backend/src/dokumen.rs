...
pub async fn upload_document(
    ...
    let result = match result {
        Ok(result) => result,
        Err(error) => {
            if let Err(cleanup_error) = state.drive.delete_file(&uploaded.id).await {
                tracing::error!(
                    drive_file_id = %uploaded.id,
                    error = %cleanup_error,
                    "Insert dokumen gagal dan file Drive hasil upload juga gagal dibersihkan"
                );
            }
...
pub async fn delete_document(
    ...
    let result = sqlx::query("DELETE FROM dokumen WHERE id = ?")
        .bind(id)
        .execute(&state.database)
        .await
        .map_err(ApiError::database)?;
        ...
        && let Err(error) = state.drive.delete_file(&file_id).await
    {
        tracing::warn!(
            ...
            error = %error,
            "Metadata dokumen sudah dihapus, tetapi file Drive gagal dibersihkan"
        );
    }
