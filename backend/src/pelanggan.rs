...
pub async fn get_next_pelanggan_code(
    ...
    loop {
        ...
        for byte in &mut buf {
            ...
            *byte = chars[(seed as usize) % chars.len()];
        }
...
pub async fn delete_customer(
    ...
    let result = sqlx::query("DELETE FROM pelanggan WHERE id = ?")
        .bind(id)
        .execute(&mut *tx)
        .await
        .map_err(ApiError::database)?;
    ...
    // Bersihkan folder setelah commit. Jika Drive gagal, data utama tetap
    // konsisten dan folder yatim dapat dibersihkan dari log operasional.
        ...
        && let Some(folder_id) = parse_drive_folder_id(&link)
    {
        if let Err(e) = state.drive.delete_file(&folder_id).await {
            tracing::warn!(
                ...
                folder_id = %folder_id,
                error = ?e,
                "Pelanggan sudah dihapus, tetapi folder Drive gagal dibersihkan"
            );
        } else {
